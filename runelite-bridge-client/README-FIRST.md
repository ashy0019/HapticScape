...
This is the fallback RuneLite launcher for HapticScape's Local Event Bridge.
It is intended for cases where the bridge is not installed through the RuneLite
Plugin Hub.

Run `HapticScape Bridge RuneLite.exe` instead of the normal RuneLite launcher,
...
fallback client; running both would duplicate the same local event stream.
...
fallback package is then unnecessary.
